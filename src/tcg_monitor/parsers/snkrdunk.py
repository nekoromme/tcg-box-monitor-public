...
_RETAILERS: tuple[tuple[str, str, tuple[str, ...]], ...] = (
    ("pokemon_center_online", "ポケモンセンターオンライン", ("ポケモンセンターオンライン",)),
    (
        "pokemon_center_store",
        "ポケモンセンター（店頭）",
        ("ポケモンセンター(店頭販売)", "ポケモンセンター（店頭販売）"),
    ),
    (
        "aeon_style_online",
        "イオン（ウェブ応募）",
        ("イオンスタイルオンライン", "イオン 東北", "イオン東北"),
    ),
    ("kids_republic", "イオンキッズリパブリック", ("キッズリパブリック",)),
    ("yamada_denki", "ヤマダデンキ", ("ヤマダデンキ", "ヤマダ電機")),
    ("yodobashi", "ヨドバシカメラ", ("ヨドバシカメラ", "ヨドバシ")),
    ("rakuten_books", "楽天ブックス", ("楽天ブックス",)),
    ("amazon_jp", "Amazon.co.jp", ("Amazon", "アマゾン")),
    ("kojima", "コジマ", ("コジマ",)),
    ("geo", "ゲオ", ("ゲオ", "GEO")),
    ("hobby_search", "ホビーサーチ", ("ホビーサーチ",)),
)
_CATEGORIES = {
...
def _retailer(value: str) -> tuple[str, str] | None:
    folded = value.casefold()
    for retailer_id, retailer_name, aliases in _RETAILERS:
    ...
    if product_title:
        ...
        for title_variant in dict.fromkeys(title_variants):
            flexible_title = r"\s*".join(
                re.escape(part)
                for part in re.split(r"\s+", title_variant)
                if part
            )
            ...
            classified = classify_product(
                config.games[game_id], product_name, product_name
            )
...
def parse_snkrdunk(
    ...
    for heading_text, block_text, links in blocks:
        retailer = _retailer(heading_text)
        if not retailer:
            continue
