...
def _priority_retailer_sections(
    ...
    sections: list[tuple[str, str, str, str, str, list[Tag], list[str]]] = []
    raw_specs = source.parser_options.get("priority_retailers", [])
    if not isinstance(raw_specs, list):
        raise ValueError(f"bad priority retailer profiles: {source.id}")
    ...
    for raw in raw_specs:
        if not isinstance(raw, dict) or not all(
            isinstance(raw.get(field), str) and raw[field] for field in required_fields
        ):
            raise ValueError(f"bad priority retailer profile: {source.id}")
        values = [str(raw[field]) for field in required_fields]
        specs.append(
            (
                values[0],
                values[1],
    ...
    for heading in soup.find_all(list(_HEADINGS)):
        ...
        spec = next(
            (
                item
                for item in specs
                if item[0] in heading_text and item[6].rstrip("/") == url.rstrip("/")
            ),
            None,
        )
        if not spec:
