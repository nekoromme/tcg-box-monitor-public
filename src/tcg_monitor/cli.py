...
from tcg_monitor.expedition import (
    ...
    load_tohoku_expedition_mode,
    tohoku_expedition_label,
)
...
_CONFIRMED_FALSE_POSITIVE_CASES: dict[str, dict[str, str]] = {
    "b3ead67cf50edb332b84384beb3bfc8c708c5eb0601e5cf95895b5f1def6043a": {
        "retailer_id": "kojima",
        "retailer_name": "コジマ",
        ...
        "event_id": (
            "tcg8c2393b32768f27eb41c378f08ad328d2c788c4c60a29f8853489510de004"
        ),
    },
    "2b4300ef9195694ee76e2b2e80acd2c73fca9b2ca63ebefd3ef41484542cc98e": {
        ...
        "event_id": (
            "tcg870cbc59d0b2aed2b3be8a58ab631f45e32a2135492b9706fad040139dd7f"
        ),
    },
    "5700dd3c057bdfd4ee9872cae4b7dc0913f385f7d57952f617493f752204042a": {
        "retailer_id": "seagull_sendai",
        "retailer_name": "シーガル各店",
        "product_name": (
            "ONE PIECEカードゲーム ブースターパック 世界最強の戦士【OP-17】"
        ),
        "start_at": "2026-08-02 10:00:00+09:00",
        "source_url": "https://nyuka-now.com/archives/97393",
        "event_id": (
            "tcge3d57f38055696b320682ac990fbed4da8fa534ef434a35d9a04237eadbcd"
        ),
    },
    # This date-only event came from the now-disabled nonofficial social
    # fallback.  SNKRDUNK article 32892 supplies the same retailer/product with
    # the exact 18:00 opening time and the official Hobby Search product URL.
    "2b5488f0479147ce1524d884875042da1e84ec46cf773a9ddc156f211642cd02": {
        ...
        "event_id": (
            "tcg2193158b6acc1392576fb442a616c9d5ad04ed7287fdc27e8e4883e9e43d4"
        ),
    },
}

_CONFIRMED_FALSE_POSITIVE_RELEASES: dict[str, dict[str, str]] = {
    "ec5c020af436d8a7cd994917b711a1094f837829544aea5bb12241fc7aa400c0": {
        "game_id": "pokemon_card",
        "product_name": "ハイクラスパック「ポケットモンスター ルビー・サファイア」",
        "release_date": "2026-07-31",
        "source_url": "https://snkrdunk.com/articles/32581/",
        "event_id": (
            "tcg7bef96ffbc518ca8b8953882e69ad1b2b2f2ef2c7a0d63862360748037d42"
        ),
    },
    "c06ffae922311fab6aebd0bc5df2ea5ea070328f3362276030949913e34d6aba": {
        "game_id": "one_piece_card",
        "product_name": "ブースターパック「世界最強の戦士」",
        "release_date": "2026-08-22",
        "source_url": "https://snkrdunk.com/articles/32599/",
        "event_id": (
            "tcged9d1a1cdaeec6893c0456f1fbfafae75c99288d564efc75491b2b6c9eab8"
        ),
    },
    "71ddfbc10aa3cb757dffcd0660c9ebc9714b451e02b5ac129582107e30ae82fe": {
        "game_id": "one_piece_card",
    ...
    "4955064739124218e139bef66f488874f123ce078fd7fabbb508eae04eebc6ad": {
        "game_id": "pokemon_card",
        "product_name": "拡張パック「30th CELEBRATION」",
        "release_date": "2026-09-16",
        "source_url": "https://www.pokemon-card.com/products/",
        "event_id": (
            "tcg02ea64042327c86f85fbcb1d498277fcfcefc32e61e86dee1a63feb970551"
        ),
    },
    # The official catalog labels this as "その他の商品".  It was briefly
    # admitted by the generic BOX evidence rule and reused the already-cleaned
    # historical event ID because both catalog cards link to /products/.
    "49550fefef3af3302da3d20b912608762c58ab036d4dbd3fff229b86820d295e": {
        "game_id": "pokemon_card",
...
def _cleanup_confirmed_false_positive_cases(
    ...
    for case_id, expected in _CONFIRMED_FALSE_POSITIVE_CASES.items():
...
def _cleanup_confirmed_false_positive_releases(
    ...
    for release_id, expected in _CONFIRMED_FALSE_POSITIVE_RELEASES.items():
        ...
        if not isinstance(release_record, dict) or (
            ...
            raise RuntimeError(
                "清掃対象発売日の監視状態が不完全です: "
                f"product={expected['product_name']}"
            )
        if not expected_event_id and sync_record is not None:
            raise RuntimeError(
                "状態のみの清掃対象に未確認の予定があります: "
                f"product={expected['product_name']}"
            )
...
def _migrate_existing_release_event_colors(
    ...
    for sync_key in sorted(calendar_sync):
        ...
        if result.get("status") not in {"updated", "not_found"}:
            raise RuntimeError(
                f"Google Calendar発売日色の更新が完了しませんでした: {result}"
            )
...
def _reuse_first_detection_start(state: MonitorState, case: LotteryCase) -> LotteryCase:
    ...
    # It repairs both SNKRDUNK's old mutable article date and Vidaway佐沼 records
    # where OCR previously promoted the winner-announcement date to a start date.
    delivery = state.data.get("delivery_journal", {}).get(
        f"lottery:started:{case.case_id}", {}
    )
    ...
    first_detection_start = (
        delivered_at.astimezone(ZoneInfo("Asia/Tokyo")).date()
        + timedelta(days=day_offset)
    )
    prepared_date = (
        prepared.start_at.date()
        if isinstance(prepared.start_at, datetime)
        else prepared.start_at
    )
...
def _prepare_cases(
    state: MonitorState, cases: list[LotteryCase]
) -> tuple[list[LotteryCase], int]:
...
def _prepare_releases(
    state: MonitorState, releases: list[Release]
...
def _summary_markdown(state: MonitorState) -> str:
    ...
    expedition_mode = str(summary.get("tohoku_expedition_mode") or "OFF")
            ...
            lines.append(
                ...
                + " | ".join(
                    (
                        ...
                        ", ".join(
                            str(value)
                            for value in record.get("healthy_fallbacks", [])
                        )
                        if (
...
def _is_amazon_invitation(case: LotteryCase) -> bool:
    return (
        ...
        and case.extraction_method in _AMAZON_INVITATION_METHODS
    )
...
def _opportunity_uses_calendar(case: LotteryCase) -> bool:
    return (
        case.opportunity_kind != OpportunityKind.DIRECT_SALE_SEEN
        and not _is_amazon_invitation(case)
    )
...
def _opportunity_is_still_open(case: LotteryCase, today: date) -> bool:
    ...
    start_date = (
        case.start_at.date()
        if isinstance(case.start_at, datetime)
        else case.start_at
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--tohoku-expedition-switch",
        default=DEFAULT_SWITCH_PATH,
        help="東北遠征モードのON/OFFファイル",
    )
    parser.add_argument(
        "--game-switch",
    ...
    try:
        ...
        tohoku_expedition_enabled = load_tohoku_expedition_mode(
            args.tohoku_expedition_switch
        )
    except (ExpeditionModeError, GameModeError) as exc:
    ...
    expedition_mode = tohoku_expedition_label(tohoku_expedition_enabled)
    expedition_source_count = sum(
        source.activation_group == TOHOKU_EXPEDITION_GROUP
    ...
    if args.cmd == "validate-config":
        print(
            ...
            f"tohoku_expedition_sources={expedition_source_count} "
            f"tohoku_expedition_mode={expedition_mode}"
        )
        return 0
    if args.cmd == "status":
    ...
    if args.cmd == "arm":
        state.arm()
        print(
            f"armed enabled_games={','.join(enabled_game_names) or 'none'} "
            f"tohoku_expedition_mode={expedition_mode}"
        )
        return 0
    ...
    source_filter = active_source_filter(
        ...
        tohoku_expedition_enabled=tohoku_expedition_enabled,
    )
    ...
    if args.cmd == "baseline":
        ...
        state.record_run_summary(
            {
                **state.data.get("last_run_summary", {}),
                ...
                "tohoku_expedition_mode": expedition_mode,
                "enabled_game_ids": sorted(config.active_game_ids),
                "tohoku_expedition_sources": (
                    expedition_source_count if tohoku_expedition_enabled else 0
                ),
            }
        )
        for case in cases:
        ...
        for release in releases:
            ...
            summary = (
                config.games[release.game_id].release_calendar_prefix
            ...
            if not state.delivered(key) or state.calendar_payload_changed(
                key, payload_hash
            ):
                ...
                if result.get("status") not in {"inserted", "updated"}:
                    raise RuntimeError(
                        f"Google Calendar登録が完了しませんでした: {result}"
                    )
    ...
    if args.cmd == "run":
        ...
        cleanup_results = [
            ...
            *_cleanup_confirmed_false_positive_releases(state, calendar),
        ]
        if cleanup_results:
            print(
                json.dumps(
        ...
        for release in releases:
            ...
            if release_calendar_result.get("status") not in {
                ...
                raise RuntimeError(
                    "Google Calendar照合が完了しませんでした: "
                    f"{release_calendar_result}"
                )
            if (
