...
def _parser_for(source_id: str):  # type: ignore[no-untyped-def]
    ...
    if is_livepocket_source(source_id):
    ...
    if is_yahoo_realtime_source(source_id):
        return parse_yahoo_realtime
    if source_id == "meli_melo_toreca_douraku_current":
        return parse_meli_melo_douraku_current
    ...
    if is_retailer_lottery_source(source_id):
...
def _fixture_path(fixture_dir: str, source_id: str, url: str) -> Path:
    if is_livepocket_source(source_id):
        ...
        if url_parts.path.startswith("/e/"):
...
def run_pipeline(
    ...
    for source in selected_sources:
        ...
        is_yahoo_source = is_yahoo_realtime_source(source.id)
        ...
        while discovery_urls:
            ...
            try:
                ...
                if is_retailer_lottery_index(source.id, url):
                    index_error = retailer_lottery_index_error(html, source.id)
                    if index_error:
                        alerts.append(
                            _alert(
                                source.id,
                ...
                if is_livepocket_source(source.id) and is_livepocket_search_page(url):
                ...
                parser = _parser_for(source.id)
                primary_without_candidates_requires_fallback = False
                if is_yahoo_realtime_source(source.id):
                    if (
                        url in primary_roots
                        and not yahoo_realtime_page_loaded(html, source.id)
                    ):
                        raise ValueError(
