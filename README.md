...
全監視先の`poll_minutes`は120分へ統一し、毎日06:04、11:04、16:04、18:04、20:04、22:04の各実行で同じ監視先集合を評価します。汎用店舗・通販・抽選サイトは、ポケカ、ワンピカード、ドラゴン、ロルカナ、遊戯王の5作品を`verified`または`prospective`で必須登録します。メーカー公式、作品公式ショップ、ポケモンセンター、作品専用の補完経路などは`coverage_scope: specialized`を明示して対象作品だけを解析します。新しい汎用監視先で5作品のどれかが欠けた場合や、個別の確認間隔を設定した場合は`validate-config`が失敗します。
