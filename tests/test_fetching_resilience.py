...
def test_parser_exception_does_not_stop_other_monitors(
    ...
    monkeypatch.setattr(
        pipeline,
        "_parser_for",
        lambda source_id: broken_parser if source_id == "broken_parser" else healthy_parser,
    )
...
def test_explicit_browser_fallback_recovers_an_http_read_timeout() -> None:
    ...
    http = FakeHttpFetcher(
        {url: HttpAttemptsExhausted(url, 3, read_timeout)}
    )
    rendered_calls: list[str] = []
...
def test_storefront_with_header_login_form_is_normal_content() -> None:
    product_rows = "".join(
        f"<li>商品{i} 在庫あり 詳細を見る</li>" for i in range(250)
    )
    html = f"""
    <html>
...
def test_http_fetcher_sends_identified_browser_accept_headers(
    ...
    class RecordingClient:
        ...
        def get(
            self, _url: str, *, headers: dict[str, str], timeout: float
        ) -> Response:
            assert timeout == 1
...
def test_hobby_station_official_news_falls_back_to_livepocket_search(
    ...
    source = next(
        item for item in config.sources if item.id == "livepocket_hobby_station"
    )
...
def test_yahoo_success_skips_twstalker_fallback() -> None:
    ...
    source = next(
        ...
        if item.id == "yahoo_realtime_tsutaya_ichinoseki"
    )
...
def test_yahoo_all_primary_queries_succeed_without_twstalker() -> None:
    ...
    source = next(
        item
        for item in config.sources
        if item.id == "yahoo_realtime_premium_bandai_onepiece"
    )
...
def test_healthy_fallback_marks_primary_degraded_without_discord_alert(
    ...
    assert state.data["monitors"]["primary"]["coverage_status"] == (
        "covered_by_fallback"
    )
...
def test_conditional_get_reuses_etag_and_last_modified() -> None:
    ...
    assert http.calls == [
        (url, '"v1"', "Sat, 25 Jul 2026 00:00:00 GMT")
    ]
...
def test_yahoo_provisional_case_is_revisited_from_detail_page(tmp_path) -> None:
    base = load_config("sites.yaml")
    source = next(
        item for item in base.sources if item.id == "yahoo_realtime_geo_official"
    )
...
def test_ocr_failure_is_pending_before_repeated_alert() -> None:
    ...
    source = next(
        item
        for item in config.sources
        if item.id == "yahoo_realtime_yorozuya_morioka"
    )
    ...
    assert [alert.reason_code for alert in third_alerts] == [
...
def test_validate_config_rejects_unknown_fallback_source(tmp_path: Path) -> None:
    original = Path("sites.yaml").read_text(encoding="utf-8")
    invalid = original.replace(
        "fallback_source_ids: [yahoo_realtime_geo_official]",
        "fallback_source_ids: [missing_source]",
        1,
    )
    path = tmp_path / "invalid-sites.yaml"
    path.write_text(invalid, encoding="utf-8")
...
def test_validate_config_requires_verified_game_fallback_coverage(
    ...
    invalid = original.replace(
        "fallback_source_ids: [yahoo_realtime_geo_official]",
        "fallback_source_ids: [snkrdunk_onepiece]",
        1,
    )
    path = tmp_path / "invalid-sites.yaml"
    path.write_text(invalid, encoding="utf-8")
...
def test_prospective_game_is_not_accepted_as_verified_fallback(
    ...
    invalid = original.replace(
        "fallback_source_ids: [yahoo_realtime_geo_official]",
        "fallback_source_ids: [yamada_denki]",
        1,
    )
    path = tmp_path / "invalid-sites.yaml"
    path.write_text(invalid, encoding="utf-8")
...
def test_access_limited_production_sources_declare_healthy_alternatives() -> None:
    ...
    assert by_id["yahoo_realtime_yamada_secondary"].source_tier == (
        SourceTier.SECONDARY
    )
    assert by_id["yahoo_realtime_kojima_secondary"].source_tier == (
        SourceTier.SECONDARY
    )
    assert by_id["pokemon_center_store"].fallback_source_ids == [
        "yahoo_realtime_pokemon_center_store"
    ]
    assert by_id["yodobashi"].fallback_source_ids == [
        "yahoo_realtime_yodobashi"
    ]
    assert by_id["konami_style_yugioh"].fallback_source_ids == [
    ...
    assert by_id["kids_republic"].supported_games["yu_gi_oh"] == (
    ...
    assert by_id["hobby_search_lottery"].fallback_source_ids == [
        "snkrdunk_pokemon"
    ]
    assert by_id["edion_online_lottery"].fallback_source_ids == [
