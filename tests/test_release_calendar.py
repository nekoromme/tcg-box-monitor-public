...
def test_browser_user_agent_keeps_chromium_shape_and_monitor_identity() -> None:
    user_agent = identified_browser_user_agent(
        "140.0.7339.16",
        "https://github.com/OWNER/tcg-box-monitor\n",
    )
...
def test_ocr_hour_range_end_is_not_parsed_as_minutes() -> None:
    ...
    assert parse_first_datetime("2026年8月7日(金) 12時20時").value == datetime(
        2026, 8, 7, 12, 0, tzinfo=timezone
    )
    assert parse_first_datetime("2026年8月7日(金) 12時30").value == datetime(
        2026, 8, 7, 12, 30, tzinfo=timezone
    )
...
def test_snkrdunk_retailer_dates_are_parsed_and_merged() -> None:
    ...
    assert not [
        alert
        for alert in alerts
        if alert.reason_code in {"start_time_not_published", "retailer_lottery_block_without_start"}
    ]
