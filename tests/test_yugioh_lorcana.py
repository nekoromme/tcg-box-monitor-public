...
def test_yugioh_catalog_keeps_regular_and_limited_boxes_only() -> None:
    ...
    html = Path("tests/fixtures/yugioh_official_products.html").read_text(
        encoding="utf-8"
    )
...
def test_lorcana_catalog_follows_only_booster_details() -> None:
    ...
    root = Path("tests/fixtures/lorcana_official_products.html").read_text(
        encoding="utf-8"
    )
    set_page = Path(
        "tests/fixtures/lorcana_official_products__attack-of-the-vine.html"
...
def test_shared_lottery_parser_recognizes_both_new_games() -> None:
    ...
    for game_id, product in samples:
        html = (
            f"<h1>{product} 抽選販売</h1>"
            "<p>販売受付期間：2026年8月20日 10:00 ～ 8月23日 23:59</p>"
        )
