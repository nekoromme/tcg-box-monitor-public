...
def test_existing_correct_case_drops_provisional_duplicate_for_same_event(
    ...
    for case_id in (correct.case_id, provisional_id):
        ...
        state.data["delivery_journal"][f"lottery:started:{case_id}"] = {"status": "complete"}
    ...
    assert f"lottery:started:{provisional_id}" not in state.data["delivery_journal"]
...
def test_unchanged_calendar_event_is_not_updated_again(
    ...
    assert discord_calls == ["【ポケカ抽選開始】テスト店舗／拡張パック「テスト」"]
...
def test_schema_v1_state_is_upgraded_without_losing_delivery_history(
        ...
        json.dumps(
            {
                ...
                "seen_releases": {},
                "ocr_cache": {},
                "delivery_journal": {"lottery:started:legacy": {"status": "complete"}},
                "alerts": {},
            }
        ),
        encoding="utf-8",
    ...
    assert reloaded.data["delivery_journal"]["lottery:started:legacy"]["updated_at"]
...
def test_state_prunes_pending_ocr_when_cache_has_recovered(tmp_path: Path) -> None:
    ...
    state.data["ocr_cache_meta"][status_url] = {"updated_at": datetime.now(UTC).isoformat()}
