...
def test_douraku_current_roundup_is_scoped_to_sendai_store() -> None:
    ...
    cases, releases, alerts = parse_curated_store_campaign(
        html,
    ...
    assert cases[0].official_url == ("https://x.com/Dourakusendai/status/2084826013847130224")
...
def test_plaza55_secondary_recovers_deadline_only_current_lottery() -> None:
    config = load_config("sites.yaml")
    source = next(
        item for item in config.sources if item.id == "yahoo_realtime_torecaplaza55_secondary"
    )
    html = """
    <div class="Tweet_TweetContainer__test">
      <p class="Tweet_body__test">
...
def test_lorcana_official_x_fallback_parses_booster_reservation() -> None:
    config = load_config("sites.yaml")
    source = next(item for item in config.sources if item.id == "yahoo_realtime_lorcana_official")
    assert source.discovery_urls == [
        (
            "https://search.yahoo.co.jp/realtime/search?"
...
def test_geo_official_x_ignores_application_label_as_product_name() -> None:
    config = load_config("sites.yaml")
    source = next(item for item in config.sources if item.id == "yahoo_realtime_geo_official")
    html = """
    <div class="Tweet_TweetContainer__test">
      <p class="Tweet_body__test">
...
def test_yahoo_repair_urls_only_revisit_provisional_product_names() -> None:
    ...
    config = load_config("sites.yaml")
    source = next(item for item in config.sources if item.id == "yahoo_realtime_geo_official")
    assert yahoo_repair_discovery_urls(source, seen_cases) == [
        (
