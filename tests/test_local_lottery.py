...
def test_hmv_livepocket_detects_current_onepiece_lottery() -> None:
    ...
    assert cases[0].start_at == datetime(2026, 8, 15, 13, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
...
def test_magi_sendai_actual_style_purchase_right_post_is_detected() -> None:
    config = load_config("sites.yaml")
    source = next(item for item in config.sources if item.id == "yahoo_realtime_magi_sendai")
    known = Release(
        "pokemon_card",
        "拡張パック「ストームエメラルダ」",
        "拡張パック",
...
def test_hmv_secondary_post_recovers_livepocket_lottery() -> None:
    config = load_config("sites.yaml")
    source = next(item for item in config.sources if item.id == "yahoo_realtime_hmv_secondary")
    html = """
    <div class="Tweet_TweetContainer__test">
...
def test_cached_ocr_clears_pending_before_non_box_filter() -> None:
    ...
    cases, _, alerts = parse_yahoo_realtime(
        ...
        date(2026, 7, 20),
        ocr_cache={
            status_url: ("ポケモンカードゲーム デッキシールド 応募受付期間 7月17日10:00から")
        },
        ocr_pending=pending,
    )
...
def test_nyuka_now_summary_recovers_current_seagull_and_edion_boxes(
    ...
    assert [case.retailer_id for case in cases].count("seagull_common") == 1
    ...
    seagull = next(case for case in cases if case.retailer_id == "seagull_common")
    assert seagull.start_at == datetime(2026, 8, 6, 10, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
    ...
    assert all(
        case.start_at == datetime(2026, 8, 7, 10, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
        for case in edion_cases
    )
    assert all(case.official_url == "https://edion-cp.com/pokeca082801/" for case in edion_cases)
    ...
    assert "確認元ページ: https://nyuka-now.com/archives/2459" in edion_notification
    ...
    migrated = {case.product_name: state.migrate_case_identity(case) for case in edion_cases}
    assert migrated["ポケモンカード ストームエメラルダ"] is None
    assert migrated["ポケモンカード メガブレイブ"] is None
    mega_symphonia = next(case for case in edion_cases if "メガシンフォニア" in case.product_name)
    ...
    assert f"lottery:started:{mega_symphonia.case_id}" in state.data["delivery_journal"]
    ...
    wrong_state.data["seen_cases"][wrong_seagull_id] = {
        ...
        "product_name": ("ONE PIECEカードゲーム ブースターパック 世界最強の戦士【OP-17】"),
...
def test_yamada_secondary_detects_app_only_lottery_with_medium_confidence() -> None:
    ...
    assert cases[0].start_at == datetime(2026, 8, 1, 10, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
    assert cases[0].source_tier == SourceTier.SECONDARY
    ...
    assert cases[0].extraction_method == ("yahoo_realtime_secondary_body_application_period")
...
def test_yamada_secondary_reports_postponement_without_false_calendar_case() -> None:
    ...
    assert not cases
    assert [alert.reason_code for alert in alerts] == ["lottery_postponed_or_cancelled"]
...
def test_hobby_station_official_news_catches_current_box_lottery() -> None:
    ...
    assert cases[0].start_at == datetime(2026, 8, 4, 12, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
...
def test_configured_policy_uses_first_detection_next_day(tmp_path: Path) -> None:
    ...
    assert (
        state.data["delivery_journal"][f"lottery:started:{first[0].case_id}"]["status"]
        == "complete"
    )
...
def test_tsutaya_ichinoseki_open_notice_survives_winner_notes_in_image() -> None:
    ...
    assert case.start_at == datetime(2026, 8, 7, 12, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
    assert case.source_url == ("https://x.com/tsutayaichi0412/status/2085531368847442214")
...
def test_image_open_notice_is_read_even_when_body_mentions_winners() -> None:
    ...
    assert not alerts
    assert len(cases) == 1
    assert cases[0].start_at == datetime(2026, 8, 7, 12, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
