...
def _source(source_id: str) -> SourceConfig:
    ...
    tier = (
        SourceTier.SECONDARY if source_id.endswith("_secondary") else SourceTier.OFFICIAL_INDIRECT
    )
    return SourceConfig(
        source_id,
        source_id,
...
def test_ministop_secondary_strictly_excludes_deck_set() -> None:
    html = """
    <div class="Tweet_TweetContainer__random">
      ...
      <time><a href="https://x.com/gamegetnavi/status/2095180835343471094">
      9月2日</a></time>
    </div>
    """
    source = next(
...
def test_ministop_secondary_excludes_confirmed_false_status_with_truncated_body() -> None:
    html = """
    <div class="Tweet_TweetContainer__random">
      <p class="Tweet_body__random">ミニストップオンライン
      ポケモンカード『30th CELEBRATION』抽選販売を実施
...
def test_other_retailer_generic_x_notice_still_requests_manual_check() -> None:
    ...
    assert not cases
    assert [alert.reason_code for alert in alerts] == ["yahoo_lottery_post_without_product"]
