...
def test_switch_reads_a_single_obvious_on_or_off_value(tmp_path: Path) -> None:
    switch = tmp_path / "TOHOKU_EXPEDITION_MODE.txt"
    switch.write_text("# 説明行\n\nOFF\n", encoding="utf-8")
    assert load_tohoku_expedition_mode(switch) is False

    switch.write_text("# 説明行\n\nON  # 有効\n", encoding="utf-8")
    assert load_tohoku_expedition_mode(switch) is True


def test_switch_fails_closed_when_missing_or_malformed(tmp_path: Path) -> None:
    switch = tmp_path / "TOHOKU_EXPEDITION_MODE.txt"
    with pytest.raises(ExpeditionModeError, match="ありません"):
        load_tohoku_expedition_mode(switch)

    switch.write_text("# 説明行\nYES\n", encoding="utf-8")
    with pytest.raises(ExpeditionModeError, match="ON または OFF"):
        load_tohoku_expedition_mode(switch)


def test_exactly_the_reviewed_one_visit_sources_are_in_expedition_group() -> None:
    config = load_config("sites.yaml")
    expedition_sources = {
        source.id: source
        for source in config.sources
        if source.activation_group == TOHOKU_EXPEDITION_GROUP
    }

    assert set(expedition_sources) == set(EXPEDITION_SOURCES)
    assert all(source.application_method == "web" for source in expedition_sources.values())
    assert all(source.required_store_visits == 1 for source in expedition_sources.values())
...
def test_morioka_batoloco_is_always_on() -> None:
    ...
    source = next(
        ...
        if item.id == "yahoo_realtime_batoloco_morioka"
    )
...
@pytest.mark.parametrize(
    ...
    [
        (source_id, account, retailer_name)
        for source_id, (account, retailer_name) in EXPEDITION_SOURCES.items()
    ],
)
def test_expedition_sources_parse_their_official_x_posts(
    source_id: str,
    account: str,
    retailer_name: str,
) -> None:
    ...
    <div class="Tweet_TweetContainer__test">
      ...
      ポケカ 拡張パック「遠征監視テスト」1BOX 抽選販売のお知らせ
...
def test_off_mode_stops_expedition_sources_before_network_access() -> None:
    config = load_config("sites.yaml")
    source_filter = active_source_filter(
        config.sources,
        set(EXPEDITION_SOURCES),
        tohoku_expedition_enabled=False,
    )
...
def test_on_mode_adds_expedition_sources_without_changing_normal_sources() -> None:
    config = load_config("sites.yaml")
    normal_sources = {
        source.id
        for source in config.sources
    ...
    on_filter = active_source_filter(
        config.sources,
        None,
        tohoku_expedition_enabled=True,
    )

    assert off_filter == normal_sources
    assert on_filter == normal_sources | set(EXPEDITION_SOURCES)
...
def test_config_rejects_expedition_sources_that_are_not_web_and_one_visit(
    ...
    with pytest.raises(
        ConfigError,
        match="must be web application and require exactly one store visit",
    ):
        load_config(path)
