...
def test_onepiece_official_shop_reads_national_article_and_application_link() -> None:
    ...
    url = (
        "https://bandainamco-am.co.jp/official_shop/onepiece-cardgame/news/important/20260810.html"
    )
    ...
    assert case.start_at == datetime(2026, 8, 12, 10, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
    assert case.end_at == datetime(2026, 8, 16, 23, 59, tzinfo=ZoneInfo("Asia/Tokyo"))
...
def test_konami_style_separates_lottery_from_period_limited_direct_sale() -> None:
    ...
    assert cases[0].start_at == datetime(2026, 8, 5, 11, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
    assert cases[0].end_at == datetime(2026, 8, 17, 23, 59, tzinfo=ZoneInfo("Asia/Tokyo"))
